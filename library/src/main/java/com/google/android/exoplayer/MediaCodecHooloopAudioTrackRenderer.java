...
/**
 * Decodes and renders audio using {@link android.media.MediaCodec} and {@link android.media.AudioTrack}.
 */
@TargetApi(16)
public class MediaCodecHooloopAudioTrackRenderer extends MediaCodecTrackRenderer {

    /**
     * Interface definition for a callback to be notified of {@link com.google.android.exoplayer.MediaCodecHooloopAudioTrackRenderer}
     * events.
     */
    public interface EventListener extends MediaCodecTrackRenderer.EventListener {

        /**
         * Invoked when an {@link android.media.AudioTrack} fails to initialize.
         *
     ...
     */
    public MediaCodecHooloopAudioTrackRenderer(SampleSource source) {
        this(source, null, true);
    }

    public MediaCodecHooloopAudioTrackRenderer(SampleSource source, float speed) {
        this(source, null, true);
        this.speed = speed;
    }
     ...
     */
    public MediaCodecHooloopAudioTrackRenderer(SampleSource source, DrmSessionManager drmSessionManager,
     ...
     */
    public MediaCodecHooloopAudioTrackRenderer(SampleSource source, Handler eventHandler,
     ...
     * @param eventListener                 A listener of events. May be null if delivery of events is not required.
     */
    public MediaCodecHooloopAudioTrackRenderer(SampleSource source, float minBufferMultiplicationFactor,
  ...
  protected boolean processOutputBuffer(long positionUs, long elapsedRealtimeUs, MediaCodec codec, ByteBuffer buffer, MediaCodec.BufferInfo bufferInfo, int bufferIndex, boolean shouldSkip) throws ExoPlaybackException {
    return processOutputBuffer(positionUs, codec, buffer, bufferInfo, bufferIndex, shouldSkip);
  }
  ...
  protected boolean processOutputBuffer(long timeUs, MediaCodec codec, ByteBuffer buffer,
        ...
        if (temporaryBufferSize == 0) {
            ...
            if (audioTrackStartMediaTimeState == START_NOT_SET) {
                ...
                long expectedBufferStartTime = audioTrackStartMediaTimeUs +
                        framesToDurationUs((long)submittedBytes / frameSize);
                if (audioTrackStartMediaTimeState == START_IN_SYNC
        ...
        if (sonic == null) {
            sonic = new Sonic(sampleRate, channelConfig);
            sonic.setSpeed(speed);
        }
        ...
        submittedBytes += ((float)bytesToWrite / speed);
        if (temporaryBufferSize == 0) {
            codec.releaseOutputBuffer(bufferIndex, false);
    ...
    /**
     * {@link android.media.AudioTrack#getPlaybackHeadPosition()} returns a value intended to be interpreted as
     ...
     *
     * @return {@link android.media.AudioTrack#getPlaybackHeadPosition()} of {@link #audioTrack} expressed as a
     * long.
     */
    ...
    private int getPendingFrameCount() {
        if (audioTrack == null) {
            ...
            int pendingFrameCount = (int)(submittedBytes / frameSize - getPlaybackHeadPosition());
