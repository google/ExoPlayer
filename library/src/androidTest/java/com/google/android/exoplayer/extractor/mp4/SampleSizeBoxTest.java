...
public class SampleSizeBoxTest extends TestCase {
    ...
    final byte[] FOUR_BIT_STZ2 = Util.getBytesFromHexString(ATOM_HEADER + "00000004" + SAMPLE_COUNT + "1234");
    final byte[] EIGHT_BIT_STZ2 = Util.getBytesFromHexString(ATOM_HEADER + "00000008" + SAMPLE_COUNT + "01020304");
    final byte[] SIXTEEN_BIT_STZ2 = Util.getBytesFromHexString(ATOM_HEADER + "00000010" + SAMPLE_COUNT + "0001000200030004");
    ...
    private void verifyParsing(Atom.LeafAtom stz2Atom) {
        ...
        assertEquals(0, box.getFixedSampleSize());

        for (int i = 0; i < box.getSampleCount(); i++) {
            assertEquals(i + 1, box.getSampleSizeAt(i));
        }
    }
}
