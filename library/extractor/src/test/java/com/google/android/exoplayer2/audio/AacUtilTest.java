...
public final class AacUtilTest {
  private static final byte[] AAC_48K_2CH_HEADER =
      Util.getBytesFromHexString("1190");
  ...
  @Test
  public void parseAudioSpecificConfig_twoCh48kAac() {
    try {
      ...
      fail("Exception thrown for a valid aac header");
    }
  }

  @Test
  public void parseAudioSpecificConfig_arbitrarySamplingFreqHeader() {
    // ISO 14496-3 1.6.2.1 allows for setting of arbitrary sampling frequency.
    try {
      AacUtil.Config aac = AacUtil.parseAudioSpecificConfig(ARBITRARY_SAMPLING_FREQ_BITS_HEADER);
      assertThat(aac.channelCount).isEqualTo(2);
      assertThat(aac.sampleRateHz).isEqualTo(15); // random frequency of 15 Hz
      assertThat(aac.codecs).isEqualTo("mp4a.40.2");
    } catch (ParserException e) {
      fail("Exception thrown for a valid aac header");
    }
  ...
  @Test
  public void parseAudioSpecificConfig_arbitrarySamplingFreqHeader_notEnoughBits() {
    // ISO 14496-3 1.6.2.1 allows for setting of arbitrary sampling frequency, but sometimes
    // the extra frequency bits are missing, make sure the code will throw an exception.
