...
void CopyPlane(const uint8_t *source, int source_stride, uint8_t *destination,
  ...
  while (height--) {
    std::memcpy(destination, source, static_cast<size_t>(destination_stride));
...
VIDEO_DECODER_FUNC(jint, ffmpegReceiveFrame, jlong jContext, jint outputMode, jobject jOutputBuffer,
  ...
  if (width != videoWidth && height != videoHeight) {
    ...
    init_result = env->CallBooleanMethod(
        ...
        0);
    LOGE("init_for_yuv_frame_method! wh [%d,%d], buffer wh [%d,%d]", frame->width,
         frame->height, width, height);
  }
    // update frame pts
  else {
    env->SetLongField(jOutputBuffer, jniContext->pts_field, frame->pts);
  }
  ...
  } else if (jniContext->rotate_degree == 180) {
  ...
  libyuv::I420Rotate(
      ...
      data, videoWidth,
      data + yLength, videoWidth / 2,
      data + yLength + uvLength, videoWidth / 2,
      frame->width, frame->height, rotate
  );
