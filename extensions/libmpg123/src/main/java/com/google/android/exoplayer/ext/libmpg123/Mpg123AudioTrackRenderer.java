 ...
 */
public final class Mpg123AudioTrackRenderer extends SampleSourceTrackRenderer
        implements MediaClock {

  /**
   * Interface definition for a callback to be notified of {@link Mpg123AudioTrackRenderer} events.
   */
  public interface EventListener {
     ...
     * @param e The corresponding exception.
     */
    void onDecoderError(Mpg123DecoderException e);
  ...
  private static final int INITIAL_INPUT_BUFFER_SIZE = 4096 * 2;
  ...
  private Mpg123OutputBuffer outputBuffer;
   ...
   */
  public Mpg123AudioTrackRenderer(SampleSource source) {
   ...
   */
  public Mpg123AudioTrackRenderer(SampleSource source, Handler eventHandler,
  ...
  /**
   * Returns whether the underlying mpg123 library is available.
   */
  public static boolean isMpg123Available() {
    return Mpg123Decoder.IS_AVAILABLE;
  }

  /**
   * Returns the version of the underlying mpg123 library if available, otherwise {@code null}.
   */
  public static String getMpg123Version() {
    return null; // isLibopusAvailable() ? Mpg123Decoder.getLibmpg123Version() : null;
  }
  ...
  protected boolean handlesTrack(MediaFormat mediaFormat) {
    return MimeTypes.AUDIO_MPEG_L2.equalsIgnoreCase(mediaFormat.mimeType);
  }
  ...
  protected void doSomeWork(long positionUs, long elapsedRealtimeUs, boolean sourceIsReady)
    ...
    if (decoder == null) {
      try {
        decoder = new Mpg123Decoder(1,1);
	  } catch (Mpg123DecoderException e) {
  ...
  private void renderBuffer() throws Mpg123DecoderException, AudioTrack.InitializationException,
    ...
    // HACK descubrir por que el data position es 4608
    // OPG
    if (outputBuffer.data.remaining() == 0) {
  ...
  private boolean feedInputBuffer(long positionUs) throws Mpg123DecoderException {
    if (inputStreamEnded) {
      return false;
    }
  ...
  private void notifyDecoderError(final Mpg123DecoderException e) {
    if (eventHandler != null && eventListener != null) {
      eventHandler.post(new Runnable() {
        @Override
        public void run() {
